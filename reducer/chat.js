export const chatReducer = (state, action) => {
    switch (action.type) {
      ...
      case 'message-reaction':{
        const {message, emojiKey} = action.payload || {};
        let reactionEvent;
        reactionEvent = async()=> await message.addReaction(message, emojiKey);
        const res = message.applyReactionEvent(reactionEvent);
        return{ ...state, res}
      }

      case 'reaction-to-delete':{
        const {message, emojiKey} = action.payload || {};
        let reactionEvent;
        reactionEvent = async()=> await message.deleteReaction(message, emojiKey);
        const res = message.applyReactionEvent(reactionEvent);
        return{ ...state, res}
      }
      case 'delete-message': {
