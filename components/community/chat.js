...
import { createChannelName } from '../../navigation/utils';
...
const Chat = props => {
    ...
    const [ongingVideoChat, setOngingVideoChat] = useState(null);
    ...
    useLayoutEffect(() => {
      const right = (
        <View style={style.headerRightContainer}>
          <Pressable activeOpacity={0.85} style={style.headerRightButton} onPress={videoCall}>
            <Iconn name="directions-walk" color="#fff" size={28} />
          </Pressable>
          <Pressable activeOpacity={0.85} style={style.headerRightButton} onPress={member}>
            <Iconn name="people" color="#fff" size={28} />
          </Pressable>
          <Pressable activeOpacity={0.85} style={style.headerRightButton} onPress={leave}>
            <Iconn name="directions-walk" color="#fff" size={28} />
          </Pressable>
        </View>
      );
    ...
    useEffect(() => {
      handler.addConnectionHandler('chat', connectionHandler);
      handler.addChannelHandler('chat', channelHandler);
      ...
      return () => {
        handler.removeConnectionHandler('chat');
        handler.removeChannelHandler('chat');
        unsubscribe.remove();
      };
    }, []);
    ...
    channelHandler.ongingVideoChat = (targetChannel, data) => {
      if (targetChannel.url === channel.url) {
        dispatch({ type: 'update-channel', payload: { data } });
      }
    };
    ...
    channelHandler.onReactionUpdated = (targetChannel,message, reactionEvent) =>{
      if (targetChannel.url === channel.url) {
        dispatch({ type: 'message-reaction', payload: {message, reactionEvent} });
      }
    };
    ...
    const videoCall = () =>{
      navigation.navigate('VideoChat', { channel, currentUser });
    }
    const joinCall = () =>{
      navigation.navigate('JoinCall', { channel, currentUser });
    }
    ...
    const leave = () => {
        Alert.alert('Leave', 'Are you going to leave this channel?', [
          { text: 'Cancel', },
          {
            text: 'OK',
            onPress: () => {
              navigation.navigate('FeaturedCommunity', {
                action: 'leave',
                data: { channel },
              });
            },
    ...
    const sendUserMessage = () => {
        if(state.input.length > 0) {
          ...
          params.sender = currentUser.userId;
          params.sendingStatus = 'pending';
    ...
    const selectFile = async () => {
      try {
        ...
        const FileMessageParams = class{
          constructor(){
            file
          }
        }
  
        const params = new FileMessageParams();
        params.file = {
          size: result.size,
          uri: result.uri,
          name: result.name,
