...
const Invite = props => {
      ...
      const right = (
        <View style={style.headerRightContainer}>
          <TouchableOpacity activeOpacity={0.85} style={style.inviteButton} onPress={invite}>
            <Iconn name="done" color="#fff" size={28} />
          </TouchableOpacity>
        </View>
      );
    ...
    useEffect(() => {
      handler.addConnectionHandler('invite', connectionHandler);
      ...
      return () => {
        ...
        handler.removeConnectionHandler('invite');
        unsubscribe.remove();
      };
    }, []);
    ...
    const invite = async () => {
      if (state.selectedUsers.length > 0) {
        ...
        try {
          if (!channel) {
            const params = new handler.GroupChannelParams();
            params.addUsers(state.selectedUsers) 
            const createdChannel = await handler.GroupChannel.createChannel(params);
            dispatch({ type: 'end-loading'});
            navigation.dispatch(
              StackActions.replace('CommunityChat', {
                currentUser,
    ...
    const refresh = () => {
      setQuery(handler.createApplicationUserListQuery());
